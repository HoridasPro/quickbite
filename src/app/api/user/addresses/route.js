...
export async function POST(request) {
  try {
    ...
    return NextResponse.json(
      { success: true, message: "Address added", id: result.insertedId },
      { status: 201 },
    );
  } catch (error) {
...
export async function PUT(request) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: "Failed to update default address" },
      { status: 500 },
    );
  }
}
...
export async function PATCH(request) {
  try {
    ...
    if (!updates || !email) {
      return NextResponse.json(
        { success: false, message: "Updates and email are required" },
        { status: 400 },
      );
    }
