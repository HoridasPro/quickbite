  ...
  useEffect(() => {
    const fetchStores = async () => {
      ...
      try {
        /**
         * API Description:
         * This endpoint fetches a list of meals based on a specific category.
         * Source: TheMealDB (Free Public API)
         */
        const categoryToFetch = activeCategory || "Seafood";
        const response = await fetch(
          `https://www.themealdb.com/api/json/v1/1/filter.php?c=${categoryToFetch}`,
        ...
        if (data.meals) {
          const formattedData = data.meals.map((item, index) => ({
            id: item.idMeal,
            name: item.strMeal,
            // --- ডাইনামিক ডেসক্রিপশন এখানে যোগ করা হয়েছে ---
            description: `Experience the finest ${item.strMeal} in town, prepared with fresh ingredients and authentic flavors from our expert chefs.`,
            location: "Gulshan " + (index % 2 === 0 ? "Banani" : "02"),
            time: Math.floor(Math.random() * 30 + 15),
            fee: Math.floor(Math.random() * 20 + 40),
            image: item.strMealThumb,
