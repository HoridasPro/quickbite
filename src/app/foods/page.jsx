...
// import ShopCardSkeleton from "@/components/ShopCardsSkeleton";
...
//                 key={option.id}
//                 onClick={() =>
//                   setSelectedSort(option.id === selectedSort ? "" : option.id)
//                 }
...
//                 key={option.id}
//                 onClick={() =>
//                   setSelectedOffer(option.id === selectedOffer ? "" : option.id)
//                 }
//                 className="flex items-center gap-3 cursor-pointer group hover:bg-gray-100 p-1 rounded-xl"
//               >
//                 <div
//                   className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition ${
//                     selectedOffer === option.id
//                       ? "bg-black border-black"
//                       : "border-gray-400"
//                   }`}
//                 >
...
//           {totalItems} {t("foodsFound")}{" "}
//           {selectedCategory && (
//             <>
//               {" "}
//               {t("forText")} "
//               {getDisplayCategoryName(selectedCategory, categories, language)}"
//             </>
//           )}{" "}
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mt-5 mb-20 items-start">
      ...
      <div className="col-span-1 lg:col-span-9 px-4 lg:px-6">
        ...
        <h2 className="font-bold text-2xl mt-10 mb-5">
          {loading ? "..." : totalItems} {t("foodsFound")}{" "}
          {selectedCategory && (
            <>
              {" "}
              {t("forText")} "
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {loading
            ? // লোডিং থাকা অবস্থায় ৯টি স্কেলিটন দেখাবে
              [...Array(9)].map((_, i) => <ShopCardSkeleton key={i} />)
            : foods.map((food) => (
                <FoodCards key={food.id || food._id} food={food} />
              ))}
        </div>
