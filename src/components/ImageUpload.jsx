...
export default function ImageUpload({ value, onChange, label }) {
  ...
  const processFile = async (file) => {
    ...
    try {
      // এখানে NEXT_PUBLIC_ প্রিফিক্স নিশ্চিত করুন
      ...
      if (data.success) {
        // display_url এর বদলে url ব্যবহার করা নিরাপদ
  ...
  return (
    <div className="space-y-2">
      ...
      {value ? (
        ...
        >
          <div
            className={`flex flex-col items-center justify-center pt-5 pb-6 transition-transform duration-200 ${isDragging ? "scale-110" : ""}`}
          >
            {isUploading ? (
              <Loader2 className="w-8 h-8 text-orange-500 animate-spin mb-2" />
            ) : (
              <UploadCloud
                className={`w-8 h-8 mb-2 transition-colors duration-200 ${isDragging ? "text-orange-500" : "text-gray-400"}`}
              />
            )}
            <p
              className={`text-sm font-medium transition-colors duration-200 ${isDragging ? "text-orange-600" : "text-gray-500"}`}
            >
              {isUploading
                ? t("uploading") || "Uploading..."
                : t("clickToUpload") || "Click or drag to upload"}
            </p>
          </div>
          <input
            type="file"
