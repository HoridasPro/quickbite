...
const ItemDetailView = ({ item }) => {
  ...
  const displayDesc = isBn && item?.descriptionBn ? item.descriptionBn : item?.description;
  const displayPrice = isBn && item?.priceBn ? item.priceBn : `Tk ${item?.price}`;
  ...
  const handleAddToCart = () => {
    ...
    if (missingRequirements?.length > 0) {
      alert(`${t("pleaseSelect")} ${missingRequirements.map((v) => v.title).join(", ")}`);
      return;
    }
    ...
    alert(`${t("addedToCart")} Tk ${orderPayload.totalPrice}`);
  };
          ...
          <div className="lg:col-span-2 space-y-6">
            <div>
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-bold text-gray-700">{averageRating > 0 ? averageRating : t("newText")}</span>
                <span className="text-gray-400 text-sm">({reviews.length} {t("reviewsText")})</span>
              </div>
              ...
              <div className="mt-6">
                <p className="text-gray-600 mb-4">{displayDesc}</p>
